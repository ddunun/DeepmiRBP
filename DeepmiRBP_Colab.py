...
def main():
    ...
    if gpus:
        try:
            ...
            print("=" * 60)
            print("❌ GPU CUDA 上下文已损坏！")
            print("   这通常是因为切换了 GPU 类型或上次运行出错。")
            print("")
            print("   请执行: Runtime -> Restart runtime")
            print("   重启后再运行本代码即可。")
            print("=" * 60)
            raise SystemExit("请先重启运行时再运行。")
    ...
    # 始终先构建全新的模型架构（避免 load_model 反序列化引发 CUDA 错误）
    print("\n🏗️ 构建 DeepmiRBP 模型架构 ...")
    ...
    # 如果存在预训练模型，加载其权重以继续训练
    if os.path.exists(PRETRAINED_MODEL_PATH):
        print(f"🔄 检测到已有模型，从 {PRETRAINED_MODEL_PATH} 加载权重 ...")
        try:
            model.load_weights(PRETRAINED_MODEL_PATH)
            print("✅ 已加载预训练权重，将在此基础上继续训练")
        except Exception as e:
            print(f"  ⚠️ 加载权重失败 (架构可能不匹配): {e}")
            print("  将使用随机初始化权重从头训练")
    else:
        print(f"  未找到预训练模型 ({PRETRAINED_MODEL_PATH})，从头开始训练")
